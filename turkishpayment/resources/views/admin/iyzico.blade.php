<div class="row g-3">
    ...
    <div class="mb-3 col-md-4">
        <label class="form-label" for="secretKeyInput">{{ trans('turkishpayment::messages.secret_key') }}</label>
        <input type="text" class="form-control @error('secret-key') is-invalid @enderror" id="secretKeyInput" name="secret-key" value="{{ old('secret-key', $gateway->data['secret-key'] ?? '') }}" required>
        @error('secret-key')
        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
        @enderror
    </div>
...
<div class="alert alert-info">
    <p class="mb-0">
        ...
        {!! trans('turkishpayment::messages.setup_instruction', [
            'callback_url' => '<code>' . route('shop.payments.notification', 'iyzico') . '</code>'
        ]) !!}
    </p>
</div>
